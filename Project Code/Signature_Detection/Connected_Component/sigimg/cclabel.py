...
def main():
    ...
    for filename in files:
        ...
        if ".jpg" in filename and "Cropped" in filename and fileSize != 0:
            ...
            xtra, thresh = cv2.threshold(np.array(grayscale), 127, 255, cv2.THRESH_BINARY_INV)
            ...
            for data in points.values():
                if len(data) > 3:
                    ...
                    mininmax = p3[1]
                    for l in data:
                        if l[0] == x and l[1] < mininmax:
                            mininmax = l[1]

                    p4 = (x, mininmax)

                    l1 = math.sqrt(((p4[0] - p1[0])**2) + ((p4[1] - p1[1])**2))
                    b = math.sqrt(((p3[0] - p2[0])**2) + ((p3[1] - p2[1])**2))
                    area = l1 * b
                    
                    if area >= 600:
                        l = [p1, p2, p3, p4]
                        cv2.rectangle(temp, p2, p4, (0,255,0), 3)
...
def cropByConnectedComponent(points, temp, filename):
    ...
    path = '/Users/miteshgandhi/Documents/my_project/cclabel/sigres'
    ...
    #     if w*h == 135200: # if w*h >= 75000 and w*h <= 150000:
